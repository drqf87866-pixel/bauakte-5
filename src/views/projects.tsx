...
export function ProjectOverviewPage({
  user, project, phases, mediaCounts, ok,
}: {
  user: User;
  project: Project;
  phases: Phase[];
    ...
    <Layout user={user} title={project.name} active='projects'>
      ...
      <div class='mb-6'>
        <div class='flex items-center justify-between mb-3'>
          <h2 class='font-bold text-sm text-slate-600 uppercase tracking-wide'>Bauphasen</h2>
              ...
              <a key={phase.id} href={documentsUrl + '?phases=' + phase.id}
                class={'flex flex-col items-center justify-center gap-0.5 rounded-xl border-2 p-3 min-h-[72px] text-center no-underline transition relative ' +
                  (phase.status === 'completed'
                    ? 'bg-white text-slate-700 border-success hover:shadow-sm'
      ...
      <div class='card'>
        <h2 class='font-bold text-sm text-slate-600 uppercase tracking-wide mb-3'>Verwalten</h2>
