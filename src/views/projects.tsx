...
export function ProjectDetailPage({
  user, project, phases, tagSummary, ok,
}: {
  user: User;
  project: Project;
  phases: Phase[];
    ...
    <Layout user={user} title={project.name} active="projects">
      <div class="mb-6">
        ...
        {tagSummary && tagSummary.length > 0 && (
          <div class="card mt-4">
            <h3 class="font-bold text-sm text-slate-600 mb-2 uppercase tracking-wide">Schlagwörter</h3>
            <div class="flex flex-wrap gap-x-3 gap-y-1">
              {tagSummary.map(item => (
                <a key={item.tag} href={"/projects/" + project.id + "/phases/" + item.phaseId + "?tag=" + encodeURIComponent(item.tag)}
                  class="text-sm text-accent hover:underline font-medium">
                  {item.tag} <span class="text-slate-400">({item.count})</span>
                </a>
              ))}
            </div>
          </div>
