...
export function ControlCenter({ user }: ControlCenterProps) {
  return (
    <div id='control-center' class='fixed inset-0 z-[60] hidden' aria-hidden='true'>
      ...
      {/* Desktop dropdown */}
      <div class='hidden md:block absolute top-header mt-2 right-4 w-72 bg-white rounded-xl shadow-xl border border-stone-200 overflow-hidden animate-fade-in' data-cc-stop>
        <div class='flex items-center gap-3 px-4 py-4 border-b border-stone-100'>
          <UserAvatar name={user.name} />
          <div class='min-w-0'>
            <div class='font-semibold text-stone-900 truncate'>{user.name}</div>
            <div class='text-sm text-stone-500 truncate'>{user.email}</div>
          </div>
        </div>
        <div class='p-2'>
          ...
          <form action='/logout' method='post' data-cc-action>
            <button type='submit'
              class='w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-error-light transition text-error font-medium text-sm'>
      ...
      <div class='md:hidden absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-xl animate-slide-up' data-cc-stop>
        <div class='flex items-center gap-3 px-5 pt-5 pb-4 border-b border-stone-100'>
          <UserAvatar name={user.name} />
          <div class='min-w-0'>
            <div class='font-semibold text-stone-900 truncate'>{user.name}</div>
            <div class='text-sm text-stone-500 truncate'>{user.email}</div>
        ...
        <div class='p-3 pb-6'>
          ...
          <form action='/logout' method='post' class='mt-1' data-cc-action>
            <button type='submit'
              class='w-full flex items-center gap-3 px-4 py-3.5 rounded-xl hover:bg-error-light transition text-error font-medium'>
